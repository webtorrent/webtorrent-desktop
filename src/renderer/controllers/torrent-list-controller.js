...
module.exports = class TorrentListController {
  ...
  prioritizeTorrent(infoHash) {
    this.state.saved.torrents
      ...
      .forEach((torrent) => {
        // Pause all active torrents except the one that started playing.
    ...
    console.log(
      "Playback Priority: paused torrents: ",
  ...
  resumePausedTorrents() {
    console.log(
      "Playback Priority: resuming paused torrents. Here's the state",
      this.state
    );
    if (
      !this.state.saved.torrentsToResume ||
      !this.state.saved.torrentsToResume.length
    )
      return;
  ...
  deleteTorrent(infoHash, deleteData) {
    const index = this.state.saved.torrents.findIndex(
      (x) => x.infoHash === infoHash
    );
  ...
  openTorrentContextMenu(infoHash) {
    ...
    menu.append(
      new remote.MenuItem({
        ...
        click: () =>
          dispatch("confirmDeleteTorrent", torrentSummary.infoHash, false),
      })
    );
    ...
    menu.append(
      new remote.MenuItem({
        ...
        click: () =>
          dispatch("confirmDeleteTorrent", torrentSummary.infoHash, true),
      })
    );
    ...
    menu.append(
      new remote.MenuItem({
        ...
        click: () =>
          clipboard.writeText(`https://instant.io/#${torrentSummary.infoHash}`),
      })
    );
