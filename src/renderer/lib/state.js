...
const defaultAnnounceList = require("create-torrent").announceList.map(
  (arr) => arr[0]
);
...
function getDefaultState() {
  ...
  return {
    ...
    prev: {
      /* used for state diffing in updateElectron() */ title: null,
      progress: -1,
      badge: null,
    },
    ...
    selectedInfoHash:
      null /* the torrent we've selected to view details. see state.torrents */,
    playing:
      getDefaultPlayState() /* the media (audio or video) that we're currently playing */,
...
async function load(cb) {
  ...
  console.log("SAVED, forced refresh", saved);
